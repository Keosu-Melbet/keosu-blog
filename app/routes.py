...
from app.models import Article, Category, BettingOdd, Match
from app.forms import ArticleForm, ContactForm, SearchForm
from app.seo_utils import generate_meta_tags
...
def keo_thom():
    ...
    odds = BettingOdd.query.filter(BettingOdd.match_date >= today, BettingOdd.match_date < tomorrow).all()
    meta_tags = generate_meta_tags(
...
def lich_thi_dau():
    ...
    matches = Match.query.filter(Match.match_date >= selected_date, Match.match_date < next_date).order_by(Match.match_date).all()
    meta_tags = generate_meta_tags(
...
@main_bp.route('/soi-keo/<int:page>')
def soi_keo(page=1):
    category = Category.query.filter_by(slug='soi-keo').first()
    if not category:
        flash('Chuyên mục không tồn tại', 'error')
        return redirect(url_for('main.index'))
    articles = Article.query.filter_by(category_id=category.id, published=True).order_by(desc(Article.created_at)).paginate(page=page, per_page=10, error_out=False)
    meta_tags = generate_meta_tags(
...
@main_bp.route('/meo-cuoc/<int:page>')
def meo_cuoc(page=1):
...
@main_bp.route('/tin-tuc/<int:page>')
def tin_tuc(page=1):
...
def article_detail(slug):
    ...
    article.views += 1
    db.session.commit()
    related_articles = Article.query.filter(Article.category_id == article.category_id, Article.id != article.id, Article.published == True).limit(3).all()
    meta_tags = generate_meta_tags(
...
def search():
    ...
    if query:
        ...
        articles = Article.query.filter_by(published=False)\
            .paginate(page=1, per_page=0, error_out=False)
